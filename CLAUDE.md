...
**The number has to be the review's own.** `block_request_verdict` collects every CTS-shaped token of the block's new lines (`line_request_refs`) and asks `korr_in_scope` about each: `'X'` as soon as one belongs to this review, `'W'` when they all name something else, `'-'` when there are none. The scope is `gt_scope_korr`, filled by the `zcl_ave_popup` constructor from what was typed on the selection screen plus the tasks it was expanded into and their parent K — `mt_filter_korrnums` is *replaced* by the task list during that expansion, so `mt_entered_korrnums` has to be added back or the entered request itself would be foreign. A number that is neither is resolved through `zcl_ave_request=>resolve_parent_k` first, so a task or a ToC written in a comment still lands in scope. An empty scope (package or plain object review) accepts everything: there is nothing to be wrong against. The finding it exists for is a request number one character off — `ER6K9A1JDT` in a review of `ER6K9A1JDL` — which reads right and is invisible without the check.
