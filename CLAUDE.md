...
**A jump returns to the page it started from, developer pages included.** `refresh_cr_object` used to reopen the object page (or the class view) whatever the reader had been looking at, so pressing the workbench badge while reading one developer's objects and coming back landed on a page that lists every author — the filter was gone and with it the point of the page. `mv_user_view_open` / `mv_decline_view_user` / `mv_reviewer_view` are kept across the recalc and restored, because `prepare_code_review` clears `mv_decline_view_user` on its way in; `rerender_cr_user_view` then rebuilds that page, and only if it cannot does the object page take over.
...
**The retrofit comparison starts where the other system stands — the review does not.** Snapshot 1 of the subtraction is normally the review pair, and that is wrong as soon as a remote system is given: a request is rarely alone, and picking the last of a series that still has to move leaves the earlier ones out of snapshot 1 while they are just as absent over there. Every one of their changed lines then comes out of the subtraction as a divergence of the reviewed request — the "ten false violations" shape. `load` therefore ends, for Code Review only (`iv_remote_in_list = abap_false`), by walking the local versions below NEW and taking the newest one whose request the other system's own version directory already records; its own korrnum and the K it resolves to are both matched, in both directions, because a ToC is recorded under one number here and the other number there. A version of the selected request itself is never taken. Nothing found leaves it initial and the review pair is used.

It lands in `ty_result-retro_old_version` and reaches only `precompute_part`'s retrofit branch, which reads that version's source for `lt_old_cmp`. **Keep-note: it used to overwrite `result-old_version`, i.e. the review pair itself.** With the other system years behind, the baseline walked back years with it: `Active -> v6`, blame naming a developer who left in 2020, code commented out under a request nobody remembers — all of it inside the review of a 2026 request. Whether a remote system was entered must not change the review by one character; only this comparison reaches further back.

`REMOTE` stays the third key field of `ZAVE_REVIEW` (`zcl_ave_popup=>load_review_payload` / `save_review_to_db` pass `mv_system`) for the other half of the reason: the row also holds that system's violations and the diffs behind them, and a second target system produces a third set. `zcl_ave_acr_repository=>has_remote_field` asks `DD03L` whether the column exists, because a dynamic `WHERE` on a missing column raises `CX_SY_DYNAMIC_OSQL_SEMANTICS` and the caller would read that as "no review saved". Only the **positive** answer is cached: the field is added while AVE runs — that is what the setup page asks for — and a remembered "no" would keep writing with the old key for the rest of the session.

**`zcl_ave_acr_repository` is the only class that names `ZAVE_REVIEW`** — `load_review_payload`, `save_review_payload`, `delete_review_payload`, plus the two `DD02L`/`DD03L` existence checks. Everywhere else the table appears only in a tooltip. That is not tidiness, it is what makes "the key is complete" checkable: a where-used on the table answers with one class, and adding a key field is a change in one place.

Keep-note: the two `DELETE`s used to sit as direct SQL in `zcl_ave_acr_workflow=>delete_and_recalc_selected` ("Delete and recalc all" and the all-selected path). When `REMOTE` became part of the key only the repository was adjusted — the assumption being that it owned the table access, which it did not. Both kept deleting by `TRKORR` alone, so a recalc started inside the review compared against another system wiped the plain review as well: approvals, comments and save history, with nothing else in AVE deleting a row and therefore no second chance. The lesson is in the architecture, not in the diligence: an assumption about where a table is touched is worth something only when one class enforces it.

**There is no two-field fallback, and that is the point.** Keep-note: `load_review_payload` / `save_review_payload` used to drop the `REMOTE` condition when the column was missing, as compatibility for a table nobody had extended yet. Nobody asked for it and it is what destroyed data: one row then answered — and was written by — both the plain review and the one compared against another system, so whichever ran last overwrote the other, silently. Now the field is required: without it nothing is read or written, `zcl_ave_popup=>show` pops the setup page (it checks `has_remote_field` next to `has_review_table`), and a refused save says so, naming the remote system.

**`norm_cmp_line` folds indentation and case, and nothing else.** Only a blank line yields an empty key and drops out of the accounting. Keep-note: comments used to be cut off first, on the grounds that a divergence living in a comment is not something the other system loses. It is precisely what it loses — the change history is written in comments, so their header entry (`* 10.08.2026 |CT770018 |ER4K9A16AT| INC3823847 …`) had an empty key, passed as expected, and the block that our move deletes never reached the violations page. Our own notes cancel out on their own: the request adds them, so they are in its diff as well.
...
- `check_part_exists`: does the object of this part still exist? Asks the table that owns the answer, not `TADIR`: `TRDIR` for programs and includes (what SE38 itself checks — an include of a function group or class has no `R3TR PROG` entry, and a deleted program can leave its `TADIR` row behind), `TFDIR` for function modules (`LIMU`, never in `TADIR`), `SEOCOMPO` **and `SEOREDEF`** for methods — a redefinition belongs to the class that declares it, so a subclass that redefines it has no `SEOCOMPO` row, and a Gateway `_DPC_EXT` class (almost nothing but redefinitions of its generated `_DPC` base) had every one of its methods reported as deleted and dropped out of the review entirely. A spelling neither table knows — an interface implementation, an alias — is ignorance, not proof: while `SEOCLASS` has the class, the method stays reviewable. `SEOCLASS` for the class pool and the class sections, `TADIR` for what really is an `R3TR` object of its own (CLAS, FUGR, DDLS, DDIC). Accepts a method name in either spelling — plain, or the VRSD one with the class padded to 30 chars in front of it.
...
**A recompute moves the review state, it does not drop it.** Approvals, declines, decline notes and comment threads are all keyed `<TYPE>~<NAME>~<n>`, where n is the block's position in the object. Anything that changes how blocks are cut renumbers them, `sanitize_review_state` then deletes every key that no longer exists, and the silent save writes the loss out — a plain Recalc cost a reviewer their approvals and comments exactly this way. `zcl_ave_acr_state=>remap_review_state` runs first, in `prepare_code_review`'s loop: the object's blocks are photographed before the recompute and matched to the fresh ones by `start_line` — a new block inherits from every old block that started inside it, which is what the merge below produces. Approvals and declines carry over only when **all** of the old blocks agreed (a block that grew must not read as approved because half of it was); comment threads always carry over and their messages are merged, because a comment is somebody's words and is not dropped to keep a number tidy. `start_line` is used because it is what an old payload carries — a content fingerprint would only help payloads written after it was introduced.

**A block never cuts an ABAP statement in two.** What ends a block is the context that follows it; while the statement is still unfinished that context belongs to the block, and so does any further change inside the same statement. Without it a call ended in one block and its parameters in the next — and since the shop convention writes the request number on the opening line, the second block read as undocumented, was counted as a finding, and had to be approved separately. `zcl_ave_acr_prepare=>update_stmt_open` is the whole rule: a blank line or a full-line comment leaves the state as it was, a line of code closes the statement when its code part (comment cut off) ends with `.`. It is fed the **new** side only — a deleted line is not part of the source the blocks are cut from — and tracked over the whole walk rather than reset per block, or a block opening with a deletion would know nothing about the statement it stands in. `c_stmt_bridge_max` (100 lines) caps it, so a statement whose end the rule fails to see cannot swallow an object.

The same decision is made in **three** places and must stay identical in all of them: `zcl_ave_acr_hunk_info=>collect` (numbering and metadata), `zcl_ave_acr_hunk_html=>collect_rows` (the html, indexed by that numbering) and `zcl_ave_acr_precompute=>collect_retrofit_hunks`. A block more or less on one side shifts every block number on the other, and the stored review is keyed on those numbers. The bridged context lines go into what is rendered, never into what `is_blank_hunk` is asked about: they are context, and a block that changes nothing has to stay one.
...
Generated code is excluded from review in two ways, both decided in `zcl_ave_acr_prepare`: `is_sap_generated_author` (version author `SAP*`, e.g. function-group framework includes) and `is_generated_class` (generated Gateway/SEGW classes `*_MPC`, `*_MPC_EXT` and `*_DPC`; `*_DPC_EXT` holds hand-written code and stays reviewable — **plus the function group main program `SAPL<area>`**, the include list SAP rewrites when a function module is added: it carries the name of whoever added it, so the author rule never sees it, and its whole content is `INCLUDE` lines nobody wrote. The method name still says "class"; renaming it to `is_generated_object` touches nine call sites and was left as a separate change). The class check runs in `zcl_ave_acr_workflow=>prepare_code_review`, in `precompute_part`, in `zcl_ave_acr_metrics=>collect` and in the Prepare picker; `zcl_ave_acr_state=>apply_saved_payload` also drops such objects from reviews saved before the exclusion existed.
...
**The number has to be the review's own, letter for letter.** `block_request_verdict` collects every CTS-shaped token of the block's new lines (`line_request_refs`) and asks `korr_in_scope` about each: `'X'` as soon as one belongs to this review, `'-'` when there are none, one of the faults below otherwise. The scope is `gt_scope_korr`, filled by the `zcl_ave_popup` constructor from `mt_entered_korrnums` (what was typed on the selection screen — `mt_filter_korrnums` is *replaced* by the task expansion, so the entered request has to be added back) plus `mt_filter_parent_korrnums` (the parent K of a task that was typed in). **Nothing is resolved on the found side.** Keep-note: it was, through `resolve_parent_k`, and the expanded child tasks were in the scope as well — which made the check blind to exactly what it exists for. `ER6K9A1JDT` and `ER6K9A1JDL` are neighbours in one number range: as long as the first was a task of the second, a review of `ER6K9A1JDL` accepted it everywhere and reported nothing. An empty scope (package or plain object review) still accepts everything: there is nothing to be wrong against.

**A task is not a request.** The convention writes the K the work moves under, not the S/R the developer typed it in, so `korr_type` reads `E070-TRFUNCTION` along with existence and `'T'` — *task, not TR* — is its own verdict. One `SELECT` per distinct number, cached in `gt_korr_exists`: a change-history header names the same one on every line.

**Which system a number belongs to is in the number.** The first three characters of a CTS number are the system it was created in, so `is_other_system_korr` needs no `P_SYS` and no call into that system — and that matters, because the reviews where nobody filled in a remote system are exactly the ones where nobody is thinking about the other system either. A number of another system is retrofitted code: verdict `'R'`, an **orange** `Retrofit` mark next to the red ones, because the reviewer wants to see it rather than be told it is wrong. Keep-note: this was first written against `gv_remote_sid` (P_SYS) and recognised nothing whenever that field was empty.

**A foreign number can be verified too, just not in `E070`.** An import records the source request in `VRSD` — the very fact the remote baseline walk lives on — so the object's own version history in this system answers "did code from that request ever arrive here?". `precompute_part` collects the korrnums of the part's versions and passes them into `collect` → `block_request_verdict`; a foreign number found among them raises the verdict from `'R'` to `'V'` and the mark from `Retrofit` to `Retrofit ✓`. Absence proves nothing — the request may have touched other objects only — so it downgrades the note instead of faulting it. Costs nothing: the version list is already in memory, and the set is built once per part.

**A number of our own SID can be verified.** `E070` is local: it either has that request or it does not, and for our own SID its absence is proof. Hence `'N'` — *no such TR*, the number is typed wrong (`ER6K9A1JDT` in a review of `ER6K9A1JDL`) — separate from `'W'`, where the request exists but belongs to somebody else. The same test is meaningless for another system's number, so `'R'` stays a note. `korr_exists` caches per number: a change-history header names the same one on every line. When both appear in one block the fact wins over the unverifiable — `'N'` before `'R'` — and ours wins over everything: a block naming one of our requests is `'X'`.

The verdict is a per-block field, `ty_hunk_info-req_ref`: `'X'` ours, `'R'` another system's, `'N'` ours but nonexistent, `'W'` ours but somebody else's, `'-'` none, **blank = no verdict**. On an object row `'N'` and `'W'` are counted together — both mean the number written into the code is not this review's. The blank is not a third opinion but the absence of one — a review saved before the check existed carries none, and a block without a verdict must not read as a failed one.
...
Rendered by `zcl_ave_acr_renderer`: `req_badge` / `hunk_req_badge` (the block mark, at the end of the block header line and in the approve cell of the full-source view — never floated: in the SAP GUI HTML control a float lands at the right edge of the content, and a wide diff table pushes it off screen, so the block reads as unmarked while the object row counts it) and `obj_descr_mark`, which emits **two independent findings** on an object row — `no header change descr` (the object gained no description at its top) and `N hunks without descr` (how many of its blocks name no request). They are two different things to act on: a request can write a proper header and still leave five blocks unannotated, or the other way round.
...
**A class section is exempt** — `comment_check_applies` answers `abap_false` for `CPUB` / `CPRO` / `CPRI` / `CLSD` and for a part named `SAPL*` (the function group main program, which `is_generated_class` keeps out of the review entirely while `ignore_generated` is on — the exemption still matters with the flag off), and the rule is applied twice: `collect` writes no verdict for such a part, and both renderers ask again, so a review prepared before the exemption stops marking sections without being recomputed. A section holds declarations, SAP regenerates it in an arbitrary order (see `zcl_ave_diff_decl`), and a `class-data:` line has nowhere to carry a change-history note — the convention lives in the method that uses the declaration. "The first block is the change description" means nothing there either: a section has no top of an object.
