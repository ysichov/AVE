...
"ORDER BY datum DESCENDING, zeit DESCENDING
  ...
  METHOD load_versions.
    ...
    " Derive date_from from the lower-bound TR/task. If p_task2 is supplied,
    " use it only for the lower bound; it must not remove versions newer than
    " the first TR.
    ...
    " Keep only the needed lower baseline:
    " - with p_task2: down to the next older K after p_task2
    " - without p_task2: down to the next older K after the first TR
    IF mv_filter_korrnum IS NOT INITIAL.
      DATA lv_lower_marker_idx TYPE i.
      DATA lv_lower_k_idx TYPE i.
      DATA(lv_lower_marker_korrnum) = COND trkorr(
        WHEN mv_filter_korrnum2 IS NOT INITIAL THEN mv_filter_korrnum2
        ELSE mv_filter_korrnum ).
      LOOP AT mt_versions TRANSPORTING NO FIELDS
        WHERE korrnum = lv_lower_marker_korrnum.
        lv_lower_marker_idx = sy-tabix.
        EXIT.
      ENDLOOP.
      IF lv_lower_marker_idx > 0.
        DATA(lv_after_marker_idx) = lv_lower_marker_idx + 1.
        LOOP AT mt_versions INTO DATA(ls_lower_k_scan)
          FROM lv_after_marker_idx WHERE trfunction = 'K'.
