...
CLASS zcl_ave_version2 DEFINITION DEFERRED.
...
"ORDER BY datum DESCENDING, zeit DESCENDING
    ...
    LOOP AT mt_hunk_threads INTO DATA(ls_thread_cur).
      DATA(ls_thread_to_save) = VALUE ty_saved_thread(
        ...
        versno_new   = COND #(
          ...
          THEN mt_hunk_info[ hunk_key = ls_thread_cur-hunk_key ]-versno_new
          ELSE ls_thread_cur-versno_new )
        versno_old   = COND #(
          WHEN line_exists( mt_hunk_info[ hunk_key = ls_thread_cur-hunk_key ] )
          THEN mt_hunk_info[ hunk_key = ls_thread_cur-hunk_key ]-versno_old
          ELSE ls_thread_cur-versno_old )
        versno_new_text = COND #(
          WHEN line_exists( mt_hunk_info[ hunk_key = ls_thread_cur-hunk_key ] )
          THEN mt_hunk_info[ hunk_key = ls_thread_cur-hunk_key ]-versno_new_text
          ELSE ls_thread_cur-versno_new_text )
        versno_old_text = COND #(
          WHEN line_exists( mt_hunk_info[ hunk_key = ls_thread_cur-hunk_key ] )
          THEN mt_hunk_info[ hunk_key = ls_thread_cur-hunk_key ]-versno_old_text
  ...
  METHOD inject_approve_btn.
    ...
    " ── Find all ACR placeholders: <!--ACR_N--> injected by diff_to_html ──
    ...
    IF lt_bm IS NOT INITIAL.
      ...
      LOOP AT lt_bm INTO DATA(ls_bm).
        ...
        " Build inline button content (replaces the <!--ACR_N--> placeholder)
        ...
        ELSEIF line_exists( mt_approved[ table_line = lv_ck ] ).
          lv_btn = |<a id="acr_c{ lv_n }"></a>| &&
          ...
          ENDIF.
          lv_btn = lv_btn && render_comment_links( lv_ck ).
        ...
        ELSEIF lv_global_ck = 'A' OR lv_global_ck = 'D'.
          ...
          lv_btn = |<a id="acr_c{ lv_n }"></a>| && lv_g_label &&
                   render_hunk_action_meta( iv_hunk_key = lv_ck iv_action = lv_global_ck ).
          IF lv_own_ck = abap_false.
            lv_btn = lv_btn &&
                     |<a href="sapevent:approve~{ lv_ck }"| &&
                     ` style="margin-left:8px;background:#27ae60;color:#fff;font-weight:bold;` &&
        ...
        " Append note rows after the </tr> that contains this placeholder,
        " then replace the placeholder itself with the button content.
        IF lv_note_html IS NOT INITIAL.
          " Find the </tr> after the placeholder and insert note rows there
        ...
        " Replace <!--ACR_N--> with button HTML inline
        DATA(lv_ph) = |<!--ACR_{ lv_n }-->|.
        REPLACE FIRST OCCURRENCE OF lv_ph IN result WITH lv_btn.
      ENDLOOP.

    ELSE.
      " ── Fallback: no ACR markers (compact mode without blame, no code-review marker) ──
      " Use '...' separator rows as hunk boundaries.
    ...
    " ── "Approve All" fixed button (top-right) ──
  ...
  METHOD show_ai_prompt.
    TYPES: BEGIN OF ty_ai_obj,
             ...
             versno_new     TYPE versno,
             versno_old     TYPE versno,
             versno_new_txt TYPE string,
             versno_old_txt TYPE string,
           END OF ty_ai_obj.
    ...
    LOOP AT mt_hunk_info INTO DATA(ls_ai_hunk).
      INSERT VALUE #(
        ...
        versno_old     = ls_ai_hunk-versno_old
        versno_new_txt = ls_ai_hunk-versno_new_text
        versno_old_txt = ls_ai_hunk-versno_old_text ) INTO TABLE lt_ai_objs.
    ENDLOOP.
    ...
    LOOP AT lt_ai_objs INTO DATA(ls_ai_obj).
      ...
      LOOP AT lt_ai_diff INTO DATA(ls_ai_diff).
        ...
        IF lv_ai_in_block = abap_true.
          DATA(lv_ai_kind) = COND string(
            ...
            ELSE `inserted` ).
          lv_ai_code = lv_ai_code && |>>> start of { lv_ai_kind } block| && lv_nl.
          IF lt_ai_del IS NOT INITIAL.
            lv_ai_code = lv_ai_code && |>>> start of deleted block| && lv_nl.
            ...
            ENDLOOP.
            lv_ai_code = lv_ai_code && |&lt;&lt;&lt; end of deleted block| && lv_nl.
          ENDIF.
          IF lt_ai_ins IS NOT INITIAL.
            lv_ai_code = lv_ai_code && |>>> start of inserted block| && lv_nl.
            ...
            ENDLOOP.
            lv_ai_code = lv_ai_code && |&lt;&lt;&lt; end of inserted block| && lv_nl.
          ENDIF.
          lv_ai_code = lv_ai_code && |&lt;&lt;&lt; end of { lv_ai_kind } block| && lv_nl.
      ...
      IF lv_ai_in_block = abap_true.
        DATA(lv_ai_last_kind) = COND string(
          ...
          WHEN lt_ai_del IS NOT INITIAL THEN `deleted`
          ELSE `inserted` ).
        lv_ai_code = lv_ai_code && |>>> start of { lv_ai_last_kind } block| && lv_nl.
        IF lt_ai_del IS NOT INITIAL.
          lv_ai_code = lv_ai_code && |>>> start of deleted block| && lv_nl.
        ...
        IF lt_ai_ins IS NOT INITIAL.
          ...
          lv_ai_code = lv_ai_code && |&lt;&lt;&lt; end of inserted block| && lv_nl.
        ENDIF.
        lv_ai_code = lv_ai_code && |&lt;&lt;&lt; end of { lv_ai_last_kind } block| && lv_nl.
      ENDIF.
      ...
      lv_ai_html = lv_ai_html &&
        ...
        |<p class="meta">{ escape( val = CONV string( ls_ai_obj-versno_old_txt ) format = cl_abap_format=>e_html_text ) }| &&
        | -&gt; { escape( val = CONV string( ls_ai_obj-versno_new_txt ) format = cl_abap_format=>e_html_text ) }</p>| &&
        |<pre>{ lv_ai_code }</pre>|.
    ENDLOOP.
...
* abapmerge 0.16.7 - 2026-05-11T17:06:08.842Z
  CONSTANTS c_merge_timestamp TYPE string VALUE `2026-05-11T17:06:08.842Z`.
